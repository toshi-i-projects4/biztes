# ビズてす セットアップ手順（v5：企業管理者ホームのサイドバー化・受験招待一括発行版）
...
- **（v5で追加）企業管理者ホームのサイドバー化・受験招待の一括発行**：`company-admin-home.html` を、ビズもんのマイページと同じ「左サイドバー＋メインコンテンツ」の作りに変更した。サイドバーの並びは、ロゴ→「企業管理者ホーム」見出し→「プライバシーポリシー」「マニュアル」（常時表示）→「ダッシュボード」「レポート」「問題編集」「受験招待（個別）」「受験招待（一括）」「企業設定」（自社が利用規約に同意しサービスが有効になるまでは非表示）→オレンジ色の「運営管理ページ」ボタン（運営権限も併せ持つアカウントのみ表示）→🚧のテスト用リンク2つ→ユーザー情報欄→ログアウトボタン、という構成。「プライバシーポリシー」（`privacy.html`）「マニュアル」（`manual.html`）「ダッシュボード」（`company-dashboard.html`）「レポート」（`company-report.html`）「🚧 ランディングページ確認用」（`landing.html`）「🚧 資料請求送付資料」（`biztes-shiryo.html`）は、いずれも中身は未着手で「準備中です」とだけ表示する仮ページ。「問題編集」は既存の`operator-question-editor.html`、「受験招待（個別）」は既存の`operator-invites.html`への導線（サイドバーから新規に追加）。「受験招待（一括）」は今回新設した`operator-invites-bulk.html`で、CSVファイルのアップロードまたはテキストの貼り付け（1行につき「氏名,メールアドレス」）で、最大300名までの受験招待をまとめて発行できる。発行前に内容と入力エラーをプレビュー確認でき、発行後は招待リンク一覧をCSVでダウンロードできる。「企業設定」は既存の`company-admin-invite.html`（自社の企業管理者を招待・管理）への導線。
...
`company-admin-home.html` のサイドバー「企業設定」からも追加招待ができます（オペレーターを介さなくてよい）。
